...
export default function TechnicianApp({ user, onLogout }) {
  ...
  async function loadMaintenanceReminders() {
    try {
      ...
      const filtered = data
        .filter(m => m.bookings?.service)
        .map(m => ({
          ...m,
          service_name: m.bookings?.service
        }))

      setMaintenanceReminders(filtered)
    } catch (err) {
      console.error(err)
    }
  }
  ...
  async function handlePhotoCapture(photoUrl) {
    ...
    const photo = { id: Date.now() + Math.random(), type: cameraMode, url: photoUrl, timestamp: new Date().toISOString() }
  ...
  async function completeJob() {
    ...
    try {
      ...
      const serviceName = selectedAppointment.service || selectedAppointment.service_name;
                ...
                {apt.status !== 'completed' && (
                  <div className="work-buttons">
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setSelectedAppointment({ ...apt, photos: apt.photos || [], work_notes: apt.work_notes || '' });
                        setWorkPhotos(apt.photos || []);
                        setNotes(apt.work_notes || '');
                        setCameraMode('after');
                        setScreen('photos');
        ...
        <main className="tech-main">
          ...
          <div className="apt-full-details">
            ...
            <div className="detail-card">
              ...
              <p><b>Phone:</b> {selectedAppointment.mobile_number} <a href={`tel:${selectedAppointment.mobile_number}`} className="btn-call">📱 Call</a></p>
            ...
            <div className="photos-grid">
              {workPhotos.map(p => (
                <div key={p.id} className="photo-item">
                  <img src={p.url} alt="" style={{ width: '100%', objectFit: 'contain', maxHeight: '300px' }} />
